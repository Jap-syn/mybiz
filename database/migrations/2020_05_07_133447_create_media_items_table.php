...
class CreateMediaItemsTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->increments('media_item_id');
			$table->integer('local_post_id')->unsigned()->index('idx_mi_local_post_id');
			$table->integer('local_post_group_id')->default(0);
			...
			$table->index(['gmb_account_id','gmb_location_id','gmb_media_key'], 'idx_mi_gmb_name');
		});
	}
