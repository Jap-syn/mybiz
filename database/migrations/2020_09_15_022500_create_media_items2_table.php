...
class CreateMediaItems2Table extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('media_items2', function(Blueprint $table)
        {
            $table->increments('media_item2_id');
            $table->integer('location_id')->unsigned()->index('idx_mi2_location_id');
            $table->integer('media_item2_group_id')->default(0);
            ...
            $table->enum('gmb_location_association_category', array('CATEGORY_UNSPECIFIED','COVER','PROFILE','LOGO','EXTERIOR','INTERIOR','PRODUCT','AT_WORK','FOOD_AND_DRINK','MENU','COMMON_AREA','ROOMS','TEAMS','ADDITIONAL'))->nullable()->comment('この場所の写真が属するカテゴリ');
            ...
            $table->index(['gmb_account_id','gmb_location_id','gmb_media_key'], 'idx_mi2_gmb_name');
        });
    }
